...
namespace spr_decision
{
  ...
  : SyncActionNode(name, config), node_(node), tf_buffer_(tf_buffer), tf_listener_(tf_listener)
  {
    ...
    subscribe_target_ = node_->get_parameter_or<bool>("subscribe_target", true);
    ...
    // msg from autoaim（subscribe_target:=false 时不创建订阅，避免无自瞄时的无效 WARN）
    if (subscribe_target_) {
      std::string target_topic_name;
      node_->get_parameter("target_topic_name",target_topic_name);
      target_sub_ = node_->create_subscription<rm_interfaces::msg::Target>(
  ...
  void Topics2Blackboard::game_state_callback_(const rm_interfaces::msg::GameState::SharedPtr msg)
  {
    ...
    setOutput<std::uint8_t>("rfid", game_state_->rfid);
  ...
  void Topics2Blackboard::check_subscriber_()
  {
    if(!game_state_)
    {
      ...
      setOutput<std::uint8_t>("rfid", 0);
      // 无裁判数据时勿置 0：0<=360 会误进终局分支；用大值表示“常规阶段/未知”
      setOutput<std::uint16_t>("stage_remain_time", 420);
