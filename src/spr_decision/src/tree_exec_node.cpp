#include "spr_decision/behaviors/attack.hpp"
#include "spr_decision/behaviors/spin.hpp"
#include "spr_decision/behaviors/nav2pose.hpp"
...
int main(int argc, char **argv)
{
  ...
  // spin_or_not node's param
  node->declare_parameter("spin_or_not_topic_name", "spin_or_not");
  node->declare_parameter("align_chassis_frame", "chassis");
  node->declare_parameter("align_chassis_angular_sign", 1.0);
  node->declare_parameter("align_chassis_heading_offset_deg", 0.0);
  node->declare_parameter("align_chassis_cmd_vel_topic", "cmd_vel_nav2_result");
  // nav2pose node's param 导航节点，使用Navigation2系统将机器人导航到目标位姿。
  node->declare_parameter("send_goal_timeout_ms",1000);
  // attack node's param  攻击行为
  ...
  factory.registerNodeType<spr_decision::Nav2Pose>("Nav2Pose", node);
  factory.registerNodeType<spr_decision::AntiAutoAim>("AntiAutoAim",node);
  ...
  factory.registerNodeType<spr_decision::PostureChange>("PostureChange",node);
