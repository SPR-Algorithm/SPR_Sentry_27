...
void FakeVelTransform::cmdVelCallback(const geometry_msgs::msg::Twist::SharedPtr msg)
{
  ...
  if (is_zero_vel) {
    // 清除缓存，避免 syncCallback 在收到零速后仍重复发布上一帧非零速度
  ...
  const nav_msgs::msg::Path::ConstSharedPtr & /*local_plan_msg*/)
{
  ...
  const geometry_msgs::msg::Twist::SharedPtr current_cmd_vel = latest_cmd_vel_;
  const bool is_zero_vel =
    std::abs(current_cmd_vel->linear.x) < EPSILON &&
    std::abs(current_cmd_vel->linear.y) < EPSILON &&
    std::abs(current_cmd_vel->angular.z) < EPSILON;
  if (is_zero_vel) {
    return;
  }
