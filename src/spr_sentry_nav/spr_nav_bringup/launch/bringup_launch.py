...
def generate_launch_description():
    ...
    declare_use_gicp_cmd = DeclareLaunchArgument(
        "use_gicp",
        default_value="True",
        description="Whether to run small_gicp_relocalization in localization launch",
    )

    declare_map_to_odom_x_cmd = DeclareLaunchArgument(
        "map_to_odom_x",
        default_value="0.0",
        description="Static map->odom x when use_gicp is False",
    )

    declare_map_to_odom_y_cmd = DeclareLaunchArgument(
        "map_to_odom_y",
        default_value="0.0",
        description="Static map->odom y when use_gicp is False",
    )

    declare_map_to_odom_yaw_cmd = DeclareLaunchArgument(
        "map_to_odom_yaw",
        default_value="0.0",
        description="Static map->odom yaw (rad) when use_gicp is False",
    )
    ...
    declare_localization_launch_file_cmd = DeclareLaunchArgument(
        ...
        default_value="localization_launch.py",
        description="Which localization launch file to include when slam is False",
    )

    declare_slam_launch_file_cmd = DeclareLaunchArgument(
        "slam_launch_file",
        default_value="slam_launch.py",
        description="Which SLAM launch file to include when slam is True",
    )

    declare_use_navigation_cmd = DeclareLaunchArgument(
        "use_navigation",
        default_value="True",
        description="Whether to include navigation_launch.py (Nav2, terrain, loam_interface)",
    )

    declare_publish_static_map_to_odom_cmd = DeclareLaunchArgument(
        "publish_static_map_to_odom",
        default_value="True",
        description="Passed to SLAM launch: publish identity map->odom static TF",
    )
        ...
        [
            ...
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(
                    PathJoinSubstitution([launch_dir, slam_launch_file])
                ),
            ...
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(
                    PathJoinSubstitution([launch_dir, localization_launch_file])
                ),
    ...
    ld.add_action(declare_localization_launch_file_cmd)
    ld.add_action(declare_slam_launch_file_cmd)
