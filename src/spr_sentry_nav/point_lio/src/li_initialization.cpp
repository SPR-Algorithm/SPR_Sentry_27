...
void imu_cbk(const sensor_msgs::msg::Imu::ConstSharedPtr & msg_in)
{
  ...
  const double timestamp_raw = get_time_sec(msg_in->header.stamp);
  const double timestamp =
    timestamp_raw - timediff_imu_wrt_lidar - time_lag_IMU_wtr_lidar - time_diff_lidar_to_imu;

  // Livox + PTP 偶发 μs~ms 级回跳；小幅回跳钳位到 last+1ns，避免初始化 deque 被反复清空
  constexpr double kImuLoopBackToleranceSec = 1e-3;
  double timestamp_use = timestamp;
  if (last_timestamp_imu >= 0.0 && timestamp_use < last_timestamp_imu) {
    const double dt_back = last_timestamp_imu - timestamp_use;
    if (dt_back <= kImuLoopBackToleranceSec) {
      timestamp_use = last_timestamp_imu + 1e-9;
      static rclcpp::Clock throttle_clock{RCL_STEADY_TIME};
      RCLCPP_WARN_THROTTLE(
        rclcpp::get_logger("li_initialization"), throttle_clock, 2000,
        "imu stamp jitter %.3f ms (raw=%.9f last=%.9f), clamped +1ns",
        dt_back * 1e3, timestamp_raw, last_timestamp_imu);
    } else {
      RCLCPP_ERROR(
        rclcpp::get_logger("li_initialization"),
        "imu loop back %.3f ms (raw=%.9f last=%.9f), drop sample",
        dt_back * 1e3, timestamp_raw, last_timestamp_imu);
      return;
    }
  }
  msg->header.stamp = get_ros_time(timestamp_use);
  timestamp_use = get_time_sec(msg->header.stamp);
  imu_deque.emplace_back(msg);
  last_timestamp_imu = timestamp_use;
  // mtx_buffer.unlock();
  // sig_buffer.notify_all();
}
